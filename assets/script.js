...
function render(sections){
  ...
  (sections||[]).forEach(sec=>{
    ...
    const gridClass = cols ? `grid cols-${cols}` : 'grid';
    ...
    (sec.items||[]).forEach(item=>{
      ...
      if(item.type==='audio'){
        ...
        // taller iframe for readability
        const f = el('iframe',{class:'plot',src:item.file,loading:'lazy',title:item.title||'Interactive plot',style:'height:600px'});
        card.append(f);

      } else if(item.type==='image'){
        const img = el('img',{src:item.file, alt:item.title||'figure', style:'max-width:100%;height:auto;border-radius:10px;display:block'});
        card.append(img);
      }
